...
void addcoupon(){
    ...
    newnode = new node1;//memory allocation to the new node
    newnode->st=ch;
    newnode->value=v;
    newnode->next=NULL;
    if(head==NULL){
...
struct node *heada = nullptr, *taila = NULL;//making head for the admin
struct node *head_s;
...
{
    newnode =new node;//memory allocated to the new node
...
void displaybill()
{
   ...
   cout<<("\t\t\t\t\t\t\tTotal price: %0.02f\n",total_price);
    ...
    {total_price=49;
       cout<<("\t\t\t\t\t\t\tMinimal Price for using our service,you will have to pay %d\n",49);
       cout<<("\t\t\t\t\t\t\tThank you!!!!!!!!!!!!!\n\n");
       cout<<("\t\t\t\t\t\t\tVisit Again :))\n\n");
    }
    if(cou>0)
   ...
   cout<<("\t\t\t\t\t   ----------------------------------------------\n");
    while(1)
    {
        ...
        switch (opt)
        {
            case 1:
                displayList(head_s);
               cout<<("\t\t\t\t\t\t\tTotal Discount Given: %d",discount);
               cout<<("\n");
                break;
            case 2:
